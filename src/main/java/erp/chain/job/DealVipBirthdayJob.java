...
public class DealVipBirthdayJob implements Job{
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException{
        LogUtils.info(LOCAL_HOST_ADDRESS + "会员生日定时任务开始执行。");
        try {
            cardService.vipBirthdayThread(new HashMap());
        } catch (Exception e) {
            LogUtils.error("会员生日定时任务执行失败", className, "execute", e);
        }
    }
}
